/* tslint:disable */
/* eslint-disable */
/**
 * Mission Management
 * ## PSID API Reference: PSID002 - Mission Management  The Mission API enables customers to create, update and delete missions. A mission is composed of one or more teams, their areas of operation and communication needs. The data can be used to issue an inquiry for matching products (see PSID001).
 *
 * The version of the OpenAPI document: 1.0.0
 *
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from "../runtime";
/**
 *
 * @export
 * @interface GeographicLocationRef
 */
export interface GeographicLocationRef {
    /**
     * unique identifier
     * @type {string}
     * @memberof GeographicLocationRef
     */
    id: string;
    /**
     * Hyperlink reference
     * @type {string}
     * @memberof GeographicLocationRef
     */
    href?: string;
    /**
     * Name of the related entity.
     * @type {string}
     * @memberof GeographicLocationRef
     */
    name?: string;
    /**
     * When sub-classing, this defines the super-class
     * @type {string}
     * @memberof GeographicLocationRef
     */
    baseType?: string;
    /**
     * A URI to a JSON-Schema file that defines additional attributes and relationships
     * @type {string}
     * @memberof GeographicLocationRef
     */
    schemaLocation?: string;
    /**
     * When sub-classing, this defines the sub-class Extensible name
     * @type {string}
     * @memberof GeographicLocationRef
     */
    type?: string;
    /**
     * The actual type of the target instance when needed for disambiguation.
     * @type {string}
     * @memberof GeographicLocationRef
     */
    referredType?: string;
}

/**
 * Check if a given object implements the GeographicLocationRef interface.
 */
export function instanceOfGeographicLocationRef(value: object): value is GeographicLocationRef {
    if (!("id" in value) || value["id"] === undefined) return false;
    return true;
}

export function GeographicLocationRefFromJSON(json: any): GeographicLocationRef {
    return GeographicLocationRefFromJSONTyped(json, false);
}

export function GeographicLocationRefFromJSONTyped(
    json: any,
    ignoreDiscriminator: boolean
): GeographicLocationRef {
    if (json == null) {
        return json;
    }
    return {
        id: json["id"],
        href: json["href"] == null ? undefined : json["href"],
        name: json["name"] == null ? undefined : json["name"],
        baseType: json["@baseType"] == null ? undefined : json["@baseType"],
        schemaLocation: json["@schemaLocation"] == null ? undefined : json["@schemaLocation"],
        type: json["@type"] == null ? undefined : json["@type"],
        referredType: json["@referredType"] == null ? undefined : json["@referredType"]
    };
}

export function GeographicLocationRefToJSON(json: any): GeographicLocationRef {
    return GeographicLocationRefToJSONTyped(json, false);
}

export function GeographicLocationRefToJSONTyped(
    value?: GeographicLocationRef | null,
    ignoreDiscriminator: boolean = false
): any {
    if (value == null) {
        return value;
    }

    return {
        id: value["id"],
        href: value["href"],
        name: value["name"],
        "@baseType": value["baseType"],
        "@schemaLocation": value["schemaLocation"],
        "@type": value["type"],
        "@referredType": value["referredType"]
    };
}
